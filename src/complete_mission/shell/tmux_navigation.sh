...
tmux send-keys -t ros_session.1 'sleep 3; roslaunch abot_bringup ros_navigation.launch' C-m
tmux send-keys -t ros_session.1 'exec bash' C-m

# 创建第二个垂直分割窗格
tmux split-window -h -t ros_session.1
tmux send-keys -t ros_session.2 'sleep 4; roslaunch complete_mission camera_ar.launch' C-m
tmux send-keys -t ros_session.2 'exec bash' C-m

# 创建第三个垂直分割窗格
tmux split-window -h -t ros_session.2
tmux send-keys -t ros_session.3 'sleep 4; roslaunch complete_mission darknet_ros.launch' C-m
tmux send-keys -t ros_session.3 'exec bash' C-m

# 创建第四个垂直分割窗格
tmux split-window -h -t ros_session.3
tmux send-keys -t ros_session.4 'sleep 4; echop' C-m
tmux send-keys -t ros_session.4 'exec bash' C-m
