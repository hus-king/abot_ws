...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    case 4:
      if (ar_lable_land(ar_track_err_max, marker.id, ALTITUDE))
      {
    ...
    // 2 ar_target
    case 5:
      if (mission_obs_avoid(target3_x, target3_y, ALTITUDE, 0))
      {
        if (lib_time_record_func(1.0, ros::Time::now()))
        mission_num = 52;
      }
    ...
    // 3 ar_target
    case 7:
      if (mission_obs_avoid(target4_x, target4_y, ALTITUDE, 0))
      {
        if (lib_time_record_func(1.0, ros::Time::now()))
        mission_num = 71;
      }
    ...
    // temp target
    case 90:
      if (mission_pos_cruise(tmp_target_x, tmp_target_y, ALTITUDE, 0, 0.2))
      {
        mission_num = 9;
      }
      break;
    ...
    // 4 ar_target
    case 9:
      if (mission_obs_avoid(target5_x, target5_y, ALTITUDE, 0))
      {
        if (lib_time_record_func(1.0, ros::Time::now()))
        mission_num = 91;
      }
    ...
    // case 2000:
    //   if (pub_ego_goal(target7_x, target7_y, ALTITUDE, 0.2, 1))
    //   {
    //     if (lib_time_record_func(1.0, ros::Time::now()))
    //     {
    //       finish_ego = false;
    ...
    case 13:
      if (target_through(second_track_pos_x, 0, ALTITUDE, 0))
        mission_num = 3000;
      break;

    case 3000:
      if (mission_pos_cruise(land_target_x, land_target_y, ALTITUDE, 0, 0.2))
      {
        if (lib_time_record_func(1.0, ros::Time::now()))
        {
          mission_num = 14;
    ...
    case 14:
      if (ar_lable_land(ar_track_err_max, land_marker_id, ALTITUDE))
      {
