...
void print_param()
{
  ...
  std::cout << "target1_flag ="<<target1_flag<<std::endl;
  std::cout << "target2_flag ="<<target2_flag<<std::endl;
  std::cout << "target3_flag ="<<target3_flag<<std::endl;
  std::cout << "target4_flag ="<<target4_flag<<std::endl;
...
int main(int argc, char **argv)
{
  ...
  nh.param<int>("target1_flag",target1_flag,0);
  nh.param<int>("target2_flag",target2_flag,0);
  nh.param<int>("target3_flag",target3_flag,0);
  nh.param<int>("target4_flag",target4_flag,0);
  ...
  target_flag.push_back(target1_flag);
  ...
  while (ros::ok())
  {
    ...
    switch (mission_num)
    {
      ...
      case 5:
        if(ego_check == false){
          ...
          if(pub_ego_goal(target5_x, target5_y, ALTITUDE, err_max_ego, 0, 0))
          {
            if (lib_time_record_func(0.5, ros::Time::now()))
            {
              mission_num = 61;
	...
	break;
      ...
      case 9: // 直接降落
        ...
        if(mission_pos_cruise(park_x, park_y, LANDING_ALTITUDE, now_yaw, err_max))
        {
