...
void print_param()
{
  ...
  std::cout << "simple_target_x: " << simple_target_x << std::endl;
  std::cout << "simple_target_y: " << simple_target_y << std::endl;
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ROS_INFO("init_position_x_take_off: %f, init_position_y_take_off: %f", init_position_x_take_off, init_position_y_take_off);
  ...
  while (ros::ok())
  {
    ...
    switch (mission_num)
    {
      ...
      case 2: // 确保进入ego_planner时的高度正确，角度正确
        if(ego_check == false){
          now_yaw = calculate_yaw(simple_target_x, simple_target_y);
          ...
          if (pub_ego_goal(simple_target_x, simple_target_y, ALTITUDE, err_max_ego, 0, 0))
          {
            if(lib_time_record_func(0.5, ros::Time::now()))
            {
                now_yaw = yaw;
