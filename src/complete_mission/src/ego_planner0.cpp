...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    switch (mission_num)
    {
      ...
      case 88: //定点穿环
      ROS_WARN("<<<<<<<<<<<<<<<<<directly flying through>>>>>>>>>>>>");
        if(mission_pos_cruise(target_ring_x,ring_back_y,RING_HEIGHT,-1.57,err_max))
        {
          mission_num = 9;
          last_request = ros::Time::now();
        }
      ...
      //穿环成功后，飞回原点
      case 9:
        if(mission_pos_cruise(target1_x,ring_back_y,ALTITUDE,-1.57,err_max))
        {
          mission_num = 91;
          last_request = ros::Time::now();
        }
