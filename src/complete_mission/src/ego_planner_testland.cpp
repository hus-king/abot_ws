  ...
  while (ros::ok())
  {
    ...
    switch (mission_num)
    {
      ...
      case 10:
        ...
        {
          if(mission_pos_cruise(park_left_x,park_left_y,ALTITUDE,0,err_max)) //changed yaw
          {
            if(lib_time_record_func(0.5,ros::Time::now())) {
              mission_num = 101;
            }
      ...
      case 100:
        if(mission_pos_cruise(park_right_x,park_right_y,0.01,0,err_max)){ //changed
            ROS_INFO("AUTO.LAND");
      ...
      case 101:
        ...
        if(mission_pos_cruise(park_left_x,park_left_y,0.01,0,err_max)){ //changed
