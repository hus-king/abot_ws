...
int main(int argc, char **argv)
{
  ...
  setpoint_raw.position.z = TANK_ALTITUDE;
  setpoint_raw.yaw = 0;
  ...
  ros::Time last_request = ros::Time::now();
   ...
   while (ros::ok())
   {
     ...
     // 当无人机到达起飞点高度后，悬停等待检测tank目标
     if (fabs(local_pos.pose.pose.position.z - TANK_ALTITUDE) < 0.1)
     {
     ...
     setpoint_raw.position.z = TANK_ALTITUDE;
     setpoint_raw.yaw = 0;
     mission_pos_cruise(0, 0, TANK_ALTITUDE, 0, 0.2);
     mavros_setpoint_pos_pub.publish(setpoint_raw);
     ros::spinOnce();
     rate.sleep();
   }
