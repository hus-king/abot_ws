...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "ego_planner_collision");
  ...
  while (ros::ok())
  {
    ...
    switch (mission_num)
    {
      ...
      case 110:
      if(pub_ego_goal(box1_x , box1_y, ALTITUDE , 0.2 , 0))
      {
        if( spin_flag > 0) {
      ...
      case 111://左下
        if(pub_ego_goal(box2_x , box2_y, ALTITUDE , 0.2 , 0))
        {
          if (lib_time_record_func(0.5, ros::Time::now())){
            mission_num = 112;
            last_request = ros::Time::now();
      ...
      case 112://左上
        if(pub_ego_goal(box3_x , box3_y, ALTITUDE , 0.2 , 0))
        {
          if (lib_time_record_func(0.5, ros::Time::now())){
            mission_num = 113;
            last_request = ros::Time::now();
      ...
      case 113://右上
        if(pub_ego_goal(box4_x , box4_y, ALTITUDE , 0.2 , 0))
        {
          if (lib_time_record_func(0.5, ros::Time::now())){
            mission_num = 114;
            last_request = ros::Time::now();
      ...
      case 114://右下
      if(pub_ego_goal(box5_x , box5_y, ALTITUDE , 0.2 , 0))
      {
        if (lib_time_record_func(0.5, ros::Time::now())){
          mission_num = 110;
          last_request = ros::Time::now();
      ...
      case 12: //识别
        if(qr_mission(target_qr_x , target_qr_y , QR_ALTITUDE , 0 ))
        {
          mission_num = 13;
          last_request = ros::Time::now();
        }
      ...
      case 13:
        if(mission_pos_cruise(target_qr_x , target_qr_y , ALTITUDE , 0 , err_max))
        {
          if (lib_time_record_func(0.5, ros::Time::now()))
          {
            ...
            ROS_INFO("------------------finish the qr mission------------------------");
          }
        }
      ...
      case 71:
        if (mission_view(target5_x, target5_y, VIEW_ALTITUDE, 0))
        {
            mission_num = 72;
            last_request = ros::Time::now();
            now_check_catapult_x = check_catapult_x;
      ...
      case 8://飞机转向，转向面向圆环
      if(mission_pos_cruise(target5_x,target5_y,RING_HEIGHT,-1.57,err_max))
      {
        if(lib_time_record_func(0.5,ros::Time::now())) {
          mission_num = 80;
          last_request = ros::Time::now();
      ...
      case 81: // 横向调整
        ...
        if( ring_mission_flag == 0 ) { //对齐
          ...
          fly_ring_x = local_pos.pose.pose.position.x + 0.05;
        ...
        if(ros::Time::now() - last_request >= ros::Duration(20.0)) {
          //mission_num = 84;
          //last_request = ros::Time::now();
          fly_ring_x = ring_back_x;
          fly_ring_y = ring_back_y;
          mission_num = 830;
          ROS_ERROR("------------------FLYING_THROUGH----------------");
          last_request = ros::Time::now();
        }
        break;
      ...
      case 830:
      if(mission_pos_cruise(fly_ring_x,target_ring_y,RING_HEIGHT,-1.57,err_max))
      {
        if(lib_time_record_func(0.5,ros::Time::now()))
        {
          mission_num = 831;
      ...
      //didn't recognize ring,return to land
      case 84 :
        if(mission_pos_cruise(target5_x,target5_y,ALTITUDE,-1.57,err_max))
        {
          mission_num = 85;
          last_request = ros::Time::now();
        }
      ...
      case 85 :
        if(mission_pos_cruise(target4_x,target4_y,ALTITUDE,-1.57,err_max))
        {
          mission_num = 86;
          last_request = ros::Time::now();
        }
      ...
      case 86 :
        if(mission_pos_cruise(target3_x,target3_y,ALTITUDE,-1.57,err_max))
        {
          mission_num = 10;
          last_request = ros::Time::now();
        }
      ...
      //穿环成功后，飞回原点
      case 9:
        if(mission_pos_cruise(target1_x,fly_ring_y,ALTITUDE,-1.57,err_max))
        {
          mission_num = 91;
          last_request = ros::Time::now();
        }
      ...
      case 91:
        if(mission_pos_cruise(target2_x,target2_y,ALTITUDE,-1.57,err_max))
        {
          mission_num = 10;
          last_request = ros::Time::now();
        }
