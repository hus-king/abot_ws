...
int box_center_x = 0;
int box_center_y = 0;
bool box_find_flag = false;
void box_center_cb(const opencv_cpp_yolov5::BoxCenter::ConstPtr &msg)
{
	box_center_x = msg->x;
	box_center_y = msg->y;
	box_find_flag = msg->flag;
}

float yolov5_track_last_time_position_x = 0;
float yolov5_track_last_time_position_y = 0;
bool yolov5_track_flag = false;
bool yolov5_track(float yaw, int reverse, float altitude, float error_max, float ctrl_coef);
bool yolov5_track(float yaw, int reverse, float altitude, float error_max, float ctrl_coef)
{
	if (yolov5_track_flag == false)
	{
		yolov5_track_last_time_position_x = local_pos.pose.pose.position.x;
		yolov5_track_last_time_position_y = local_pos.pose.pose.position.y;
		yolov5_track_flag = true;
	...
	if (box_find_flag)
	{
		...
		// box_center_x 从左到右变大， box_center_y 从上到下变大
		if (box_center_x - 320 >= error_max)
		{
			setpoint_raw.position.y = local_pos.pose.pose.position.y - ctrl_coef * reverse;
		}
		else if (box_center_x - 320 <= -error_max)
		{
			setpoint_raw.position.y = local_pos.pose.pose.position.y + ctrl_coef * reverse;
		}
		else
		...
		if (fabs(box_center_x - 320) < error_max)
		{
	...
	{
		setpoint_raw.position.x = yolov5_track_last_time_position_x;
		setpoint_raw.position.y = yolov5_track_last_time_position_y;
	}
