...
float box_target_x;
float box_target_y;
float fx = 513.05889266;
float fy = 512.73391627;
float cx = 318.42476147;
float cy = 243.01191913;

std::string target_data[3] = {"car","bridge","tank"};
    ...
    {        
		...
		for(string target:target_data){
			if(target == bounding_box.Class)
			{
				...
				box_target_x = (cy - center_y) * local_pos.pose.pose.position.z / fy;
                box_target_y = (cx - center_x) * local_pos.pose.pose.position.z / fx;
				break;
			}
