...
void qr_cb(const std_msgs::String::ConstPtr& msg){
	ROS_INFO("Received QR Code Dta:%s",msg->data.c_str());
	for(int i=0;i<3;i++){
	    qr_data[i] = msg->data;
	}
}
...
bool qr_mission(float x,float y,float z,float yaw,std::string* read_data);
bool qr_mission(float x,float y,float z,float yaw,std::string* read_data){
	setpoint_raw.type_mask = /*1 + 2 + 4 */ +8 + 16 + 32 + 64 + 128 + 256 + 512 /*+ 1024 */ + 2048;
	setpoint_raw.coordinate_frame = 1;
	setpoint_raw.position.x = x + init_position_x_take_off;
	...
	std::cout << "read_data[0]:" << read_data[0] << "read_data[1]:" << read_data[1] << "read_data[2]:" << read_data[2] << std::endl;
...
float ring_y = 0;
float ring_z = 0;
bool ring_mission(float x,float y,float z,float yaw,float err);
bool ring_mission(float x,float y,float z,float yaw,float err){
	setpoint_raw.type_mask = /*1 + 2 + 4 */ +8 + 16 + 32 + 64 + 128 + 256 + 512 /*+ 1024 */ + 2048;
	setpoint_raw.coordinate_frame = 1;
	setpoint_raw.position.x = x + init_position_x_take_off;
...
//3、函数定义
*************************************************************************/
float mission_view_last_position_x = 0;
float mission_view_last_position_y = 0;
float catapult_x = 0.0;
float catapult_y = 0.0;
bool mission_view_flag = false;
bool mission_view(float x, float y, float z, float yaw, float error_max);
bool mission_view(float x, float y, float z, float yaw, float error_max)
{
	if (mission_pos_cruise_flag == false)
	{
		mission_view_last_position_x = local_pos.pose.pose.position.x;
		mission_view_last_position_y = local_pos.pose.pose.position.y;
		mission_view_flag = true;
	}
	setpoint_raw.type_mask = /*1 + 2 + 4 */ +8 + 16 + 32 + 64 + 128 + 256 + 512 /*+ 1024 */ + 2048;
	setpoint_raw.coordinate_frame = 1;
	setpoint_raw.position.x = x + init_position_x_take_off;
